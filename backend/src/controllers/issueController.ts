...
async function allocateFromStock(materialId: number, needQty: number) {
  if (needQty <= 0) return [] as { StockId: number; Barcode: string; take: number }[];
  const stocks = await prisma.stock.findMany({
    where: { MaterialId: materialId, Remain: { gt: 0 } },
...
export async function createIssueFromRequest(req: Request, res: Response) {
  try {
    ...
    const reqRow = await prisma.withdrawnRequest.findUnique({
    ...
    const existedIssue = await prisma.issue.findUnique({ where: { RequestId: requestId } });
    ...
    for (const d of details) {
      const plan = await allocateFromStock(d.MaterialId, d.WithdrawnQuantity);
...
export async function listIssues(_req: Request, res: Response) {
  try {
    const rows = await prisma.issue.findMany({ orderBy: { IssueDate: 'desc' } });
...
export async function deleteIssue(req: Request, res: Response) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const details = await tx.issueDetail.findMany({ where: { IssueId: id } });
      for (const d of details) {
        // Find stock row by barcode
        const stock = await tx.stock.findFirst({ where: { Barcode: d.Barcode } });
        if (stock) {
          await tx.stock.update({
      ...
      await tx.issueDetail.deleteMany({ where: { IssueId: id } });
      await tx.issue.delete({ where: { IssueId: id } });
    });
